...
export default function Home() {
  ...
  return (
    <main
      className={`flex min-h-screen flex-col items-center justify-center bg-[#121212] relative overflow-hidden grid-dots ${isRedirecting ? "animate-glitch" : ""}`}
    >
      {/* Sound toggle button */}
      <button
        onClick={toggleMute}
      ...
      <div className="w-[280px] h-[320px] bg-[#1e272e] rounded-[20px] relative flex flex-col items-center p-4 shadow-lg border border-[#2ed573]/30">
        ...
        <div className="w-[220px] h-[120px] bg-[#0f0f0f] mt-6 rounded-[10px] flex items-center justify-center overflow-hidden border border-[#2ed573]/20">
          <div
            className={`text-[#2ed573] text-center px-4 transition-opacity duration-500 ${isOn ? "opacity-100 terminal-text" : "opacity-0"} ${isAnimating ? "opacity-0" : "opacity-100"}`}
          >
        ...
        <button
          ...
          className={`w-[60px] h-[60px] rounded-full mt-6 flex items-center justify-center transition-all duration-300 ${isOn ? "bg-[#ffdd59]" : "bg-[#2f3542]"} hover-scale`}
          aria-label="Toggle device"
        >
          <div className="w-[50px] h-[50px] rounded-full bg-[#1e272e] flex items-center justify-center">
            <div
              className={`w-[40px] h-[40px] rounded-full ${isOn ? "bg-[#2ed573]" : "bg-[#747d8c]"} flex items-center justify-center`}
            >
